...
date_pattern = re.compile(r"""^(.*?)  # all text before the date, '?' = optional
       ((0|1)?\d)-                    # one or two digits for the month, like 01,02,11,12,1,2...
       ((0|1|2|3)?\d)-                # one or two digits for the day, like 01,1,02,2,30,31...
       ((19|20)\d\d)                  # four digits for the year, like 1990, 2018...
       (.*?)$                         # all text after the date, optional
     """, re.VERBOSE)

# Loop over the files in the working directory. '.' is CWD.
